...
use std::{collections::{HashMap, HashSet}, ptr, sync::{Mutex, OnceLock}};
use wasapi::{initialize_mta, AudioCaptureClient, AudioClient, Device, DeviceEnumerator, Direction, Handle, SampleType, SessionState, StreamMode, WaveFormat};
use windows::{core::{PCWSTR, PWSTR}, Win32::{
    Foundation::{CloseHandle, HANDLE, MAX_PATH},
    Storage::FileSystem::{GetFileVersionInfoSizeW, GetFileVersionInfoW, VerQueryValueW},
    System::{Diagnostics::ToolHelp::{CreateToolhelp32Snapshot, Process32FirstW, Process32NextW, PROCESSENTRY32W, TH32CS_SNAPPROCESS}, Threading::{GetExitCodeProcess, OpenProcess, QueryFullProcessImageNameW, PROCESS_NAME_WIN32, PROCESS_QUERY_LIMITED_INFORMATION}, },
}};

use crate::audio::cast_slice;
use crate::capture::{AppInfo, AppTarget, AudioDeviceInfo, CaptureFormat, CaptureStream};
use crate::log_info;
...
pub fn output_format(sel: &AudioDeviceSelector) -> Result<CaptureFormat> {
    initialize_mta().ok().context("initialize_mta failed (COM init; avoid calling from STA UI thread)")?;
    let enumerator = DeviceEnumerator::new()?;
    let device = select_render_device(&enumerator, sel)?;
    let format = device.get_iaudioclient()?.get_mixformat()?;
    Ok(CaptureFormat { sample_rate: format.get_samplespersec(), channels: format.get_nchannels() as usize, channel_mask: format.get_dwchannelmask(), })
}

pub fn enumerate_devices() -> Result<Vec<AudioDeviceInfo>> {
    initialize_mta().ok().context("initialize_mta failed (COM init; avoid calling from STA UI thread)")?;
    let enumerator = DeviceEnumerator::new().context("could not open the audio device enumerator")?;
    Ok(render_devices(&enumerator))
}

fn render_devices(enumerator: &DeviceEnumerator) -> Vec<AudioDeviceInfo> {
    let default_id = enumerator.get_default_device(&Direction::Render).ok().and_then(|d| d.get_id().ok());
    let Ok(coll) = enumerator.get_device_collection(&Direction::Render) else { return Vec::new() };

    let mut out = Vec::new();
    for dev_res in &coll {
        let Ok(dev) = dev_res else { continue };
        let id = dev.get_id().unwrap_or_else(|_| "<no id>".to_string());
        out.push(AudioDeviceInfo { is_default: default_id.as_deref() == Some(id.as_str()), layout: Some(describe_layout_of(&dev)), is_loopback: true, id, name: dev.get_friendlyname().unwrap_or_else(|_| "<no name>".to_string()) });
    }
    out
}

fn list_render_devices(enumerator: &DeviceEnumerator) {
    log_info!("--- Render devices (Output) ---");
    for dev in render_devices(enumerator) {
        log_info!("  - {}\n    id: {}\n    layout: {}", dev.name, dev.id, dev.layout.unwrap_or_default());
    }
}
...
pub fn open_app(target: &AppTarget, wanted: &CaptureFormat) -> Result<Box<dyn CaptureStream>> {
    initialize_mta().ok().context("initialize_mta failed (COM init; avoid calling from STA UI thread)")?;
    // The mask travels with the format, so a 7.1 capture comes back as 7.1 rather than as eight channels of unknown purpose.
    let mask = (wanted.channel_mask != 0).then_some(wanted.channel_mask);
    let format = WaveFormat::new(32, 32, &SampleType::Float, wanted.sample_rate.max(8_000) as usize, wanted.channels.clamp(1, 8), mask);
    // `true` is INCLUDE_TARGET_PROCESS_TREE. `false` would be EXCLUDE_TARGET_PROCESS_TREE, which captures
    // every other application instead of this one. Windows has no "this process only" mode, so a capture
    // always covers the target's children too; `resolve_targets` drops descendants to avoid mixing twice.
    let mut client = AudioClient::new_application_loopback_client(target.pid, true).map_err(|e| anyhow!("process loopback capture needs Windows 10 2004 or newer: {e}"))?;
    ...
    match apps.mode {
        AppMatchMode::Include => {
            for group in apps.active_groups() {
                let matched: HashSet<u32> = table.iter().filter(|p| p.pid != 0 && p.pid != own && matches_group(group, p)).map(|p| p.pid).collect();

                // A capture covers the target's whole process tree, so only the topmost match of a tree gets
                // a client. Without this a browser, whose parent and its renderers share one executable name,
                // would be mixed in once per process.
                for proc in &table {
                    if !matched.contains(&proc.pid) || has_ancestor_in(proc.pid, &matched, &parents) {
                        continue;
                    }
        ...
        AppMatchMode::Exclude => {
            ...
            // Everything that plays and is not excluded, again reduced to the topmost process of each tree.
            let playing: Vec<u32> = render_session_pids()?
                .into_iter()
                .map(|(pid, _playing)| pid)
                .filter(|&pid| pid != 0 && pid != own && !excluded.contains(&pid) && !has_ancestor_in(pid, &excluded, &parents))
                .collect();
            let roots: HashSet<u32> = playing.iter().copied().collect();

            for pid in playing {
                if has_ancestor_in(pid, &roots, &parents) {
                    continue;
                }
