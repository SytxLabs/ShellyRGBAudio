...
impl WledDevice {
    pub fn new(cfg: WledConfig) -> Result<Self> {
        ...
        {
            log_warn!("Warn: wled {}: {:?} can only address {} pixels, using that instead of {asked}.", cfg.host, cfg.protocol, cfg.protocol.max_pixels());
        }
...
impl Device for WledDevice {
    ...
    fn apply_segments(&self, frame: &Frame, segments: &[Rgbw]) -> Result<()> {
        ...
        if let Err(e) = self.sock.send(&packet) {
            log_error!("wled {} send failed (ignored): {e}", self.target);
        }
        Ok(())
    }
}
