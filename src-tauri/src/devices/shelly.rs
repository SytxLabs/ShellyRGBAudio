...
impl ShellyController {
    ...
    fn set_gen1_rgbw2(&self, r: u8, g: u8, b: u8, w: u8, gain_0_100: u8, transition_ms: u32, ) -> Result<()> {
        ...
        let resp = match req.send() {
            ...
            Err(e) => {
                if e.is_timeout() {
                    log_warn!("Gen1 Shelly timeout (ignored): {e}");
                    return Ok(());
                }
        ...
        if !resp.status().is_success() {
            log_warn!("Gen1 Shelly HTTP error (ignored): {}", resp.status());
            return Ok(());
        }
        Ok(())
    }
    ...
    fn rpc_call(&self, method: &str, params: Value) -> Result<Value> {
        ...
        let resp = match self.http.post(&url).header(CONTENT_TYPE, "application/json").body(body_bytes.clone()).send()
        {
            ...
            Err(e) => {
                if e.is_timeout() {
                    log_warn!("Gen2 Shelly timeout (ignored) POST /rpc: {e}");
                    return Ok(Value::Null);
                }
                if e.is_connect() {
                    log_warn!("Gen2 Shelly connect error (ignored) POST /rpc: {e}");
                    return Ok(Value::Null);
                }
