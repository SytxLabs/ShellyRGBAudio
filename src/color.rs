...
pub struct ColorMapConfig {
    ...
    /// How far the mixed color is pushed back to full saturation, `0.0` to `1.0`.
    /// The band energies normally sit close together because each band is normalized against its own rolling peak. Mixing them therefore
    /// lands near gray, and gray carries no hue. Pushing the spread between the weakest and strongest channel back out to the full range
    /// turns those small differences into real color again, which is what the fixed hue math did before the mapping was configurable.
    #[serde(default = "d_one")]
    pub saturation: f32,
