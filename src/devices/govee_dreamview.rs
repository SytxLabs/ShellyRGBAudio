...
impl DreamViewDevice {
    pub fn new(mut cfg: DreamViewConfig) -> Result<Self> {
        ...
        if asked > MAX_SEGMENTS {
            eprintln!("Warn: govee_dreamview {}: Govee's segment bitmap holds at most {MAX_SEGMENTS} segments, using that instead of {asked}.", cfg.ip);
        }
        ...
        if cfg.segments > 1 {
            eprintln!("Note: govee_dreamview {} is set to {} segments. Segment addressing over LAN is not part of Govee's published API and may be ignored by your model; set \"segments\": 1 if the strip does not follow.", cfg.ip, cfg.segments);
        }
