//! Room geometry: where the speakers are, where the lights are, and how much of each channel a light should hear.
//!
//! Listener at the origin, metres, `+X` right, `+Y` up, `+Z` in front of the listener:
//! ```text
//!         +Y up
//!          |
...
//! ```
//!
//! The horizontal axes come from real capture channels, decoded from the WASAPI channel mask. The vertical axis usually cannot: a stereo output
//! device carries no height information at all. Height is therefore mapped onto *frequency* instead - a light near the floor leans toward the bass
//! bands, one near the ceiling toward the treble bands. When the layout does carry height speakers (Atmos), that fallback switches itself off and
//! the vertical axis becomes real as well.
...
/// One channel of the capture format. The variants mirror the `SPEAKER_*` bits of a Windows `dwChannelMask`.
...
pub enum SpeakerRole {
    ...
    TopBackCenter,
    TopBackRight,
    /// A channel the mask did not name. Treated as omnidirectional rather than guessed at.
    Unknown,
}

/// Only WASAPI reports a channel mask; the cpal backends infer the layout from the channel count instead.
...
impl SpeakerRole {
    /// Unit direction from the listener toward the speaker. The height row sits at `y = 0.707` with its horizontal part scaled to match.
...
impl SpeakerLayout {
    /// Fallback layouts for the common channel counts, used whenever no usable mask is available.
    ...
    /// Applies the `spatial.layout` setting: `"auto"` keeps what the backend reported, an explicit list overrides it.
    pub fn resolve(self, selector: &LayoutSelector, warnings: &mut Vec<String>) -> Self {
        ...
        if roles.len() != self.channels.len() {
            warnings.push(format!(
                "spatial.layout names {} channels but the capture format has {}, using the detected layout {} instead",
                roles.len(),
                self.channels.len(),
                self.describe()
            ));
...
/// The spatial part of a `devices[]` entry. Parsed in `devices::mod`, next to the color overrides, so every device type inherits it for free.
///
/// A device is only positioned once it states either a `position` or a `path`. Until then every field here is inert and the device follows the global
/// mix, exactly as it did before positions existed.
...
pub struct DeviceSpatialConfig {
    ...
    /// The corners a strip actually follows, for one that bends: floor to ceiling and then back along the ceiling is three points. Overrides
    /// `position` and `extent`. Length spreads segments along it, so a leg twice as long gets twice as many of them.
...
impl DeviceSpatialConfig {
    ...
    /// How many points the geometry should be sampled at: one for a lamp, one per segment, or `strip_samples` for anything with length.
...
/// Everything a device needs at render time, precomputed once at startup: one weight per capture channel, one tilt per band, one brightness gain.
#[derive(Debug, Clone)]
pub struct DeviceSpatial {
    ...
    // The same two, kept per sample point instead of averaged, so an addressable device can color each segment from where that segment actually is.
    // One entry per segment, in the order they run along the device.
...
/// Mixes the per-channel band energies down through one set of channel weights, applies the height tilt, and blends the result against the global mix.
...
/// A lamp is one point; a strip is `samples` points spread evenly across its segment. Averaging over those points is the whole reason a strip reads
/// wider and smoother than a lamp - nothing else in the pipeline treats the two differently.
...
/// Per band, how strongly this device's height favors that band's frequency. All `1.0` when the mapping is off or would be meaningless.
